...
class KarmaMemberService:
    ...
    def upsert_karma_member(self, member: KarmaMember) -> UpdateResult:
        ...
        # return update result
        return self._karma.update_one(
            filter=member_dict, update=self._increase_karma, upsert=True
        )
    ...
    def delete_all_karma(self, member: KarmaMember) -> DeleteResult:
        ...
        return self._karma.delete_many(
            filter=dict(guild_id=member.guild_id, member_id=member.member_id)
        )
    ...
    def aggregate_member_by_channels(self, member: KarmaMember):
        ...
        pipeline = [
            ...
            {
                "$group": {
                    "_id": {"member_id": "$member_id", "channel_id": "$channel_id"},
                    "karma": {"$sum": "$karma"},
                }
            },
            {"$sort": {"karma": -1}},
    ...
    def aggregate_top_karma_members(
        self, guild_id: str, channel_id: str = "", time_span: int = 0
    ):
        """
...
class BlockerService:
    ...
    def blacklist(self, member: Member) -> UpdateResult:
        ...
        return self._blacklist.update_one(
            filter=member_dict, update={"$set": member_dict}, upsert=True
        )
    ...
    def find_all_blacklisted(self, guild_id):
        ...
        """
        return self._blacklist.find(
            filter=dict(guild_id=guild_id), projection=dict(member_id=True)
        )
