...
class KarmaMemberService:
    ...
    def upsert_karma_member(self, member: KarmaMember):
        ...
        return self._karma.update_one(filter=member_dict, update=self._increase_karma, upsert=True)

    def delete_karma_member(self, member: KarmaMember):
        # return delete result
        return self._karma.delete_one(
            filter=dict(guild_id=member.guild_id, member_id=member.member_id, channel_id=member.channel_id, message_id=member.message_id)
        )
    ...
    def aggregate_member_by_karma(self, member: KarmaMember):
        pipeline = [
            {"$unwind": "$karma"},
            {"$match": dict(member_id=member.member_id, guild_id=member.guild_id)},
    ...
    def aggregate_member_by_channels(self, member: KarmaMember):
        pipeline = [
            {"$unwind": "$karma"},
            {"$match": dict(member_id=member.member_id, guild_id=member.guild_id)},
            {"$group": {"_id": {"member_id": "$member_id", "channel_id": "$channel_id"}, "karma": {"$sum": "$karma"}}},
            {"$sort": {"karma": -1}},
            {"$limit": int(profile()["channels"])},
        ]
        doc_cursor = self._karma.aggregate(pipeline)
    ...
    def aggregate_top_karma_members(self, guild_id: str, channel_id: str = "", time_span: int = 0):
        if channel_id == "":
            if time_span == 0:
                ...
                pipeline = [
                    {"$unwind": "$karma"},
                    {"$match": {"guild_id": guild_id, "created_date": {"$gt": datetime.datetime.utcnow() - datetime.timedelta(days=time_span)}}},
                    {"$group": {"_id": {"member_id": "$member_id"}, "karma": {"$sum": "$karma"}}},
                    {"$sort": {"karma": -1}},
                    {"$limit": int(config["leaderboard"])},
                ]
            ...
            if time_span == 0:
                pipeline = [
                    {"$unwind": "$karma"},
                    {"$match": dict(guild_id=guild_id, channel_id=channel_id)},
                    {"$group": {"_id": {"member_id": "$member_id", "channel_id": "$channel_id"}, "karma": {"$sum": "$karma"}}},
                    {"$sort": {"karma": -1}},
                    {"$limit": int(config["leaderboard"])},
                ]
                ...
                pipeline = [
                    {"$unwind": "$karma"},
                    {
                        "$match": {
                            ...
                            "created_date": {"$gt": datetime.datetime.utcnow() - datetime.timedelta(days=time_span)},
                        }
                    },
                    {"$group": {"_id": {"member_id": "$member_id", "channel_id": "$channel_id"}, "karma": {"$sum": "$karma"}}},
                    {"$sort": {"karma": -1}},
    ...
    def aggregate_top_karma_channels(self, guild_id: str, time_span: int = 0):
        if time_span == 0:
            pipeline = [
                {"$unwind": "$karma"},
                {"$match": dict(guild_id=guild_id)},
                {"$group": {"_id": {"channel_id": "$channel_id"}, "karma": {"$sum": "$karma"}}},
                {"$sort": {"karma": -1}},
                {"$limit": int(config["leaderboard"])},
            ]
            doc_cursor = self._karma.aggregate(pipeline)
