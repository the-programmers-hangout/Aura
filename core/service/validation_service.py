...
async def contains_valid_thanks(message: str) -> bool:
    ...
    for thanks in thanks_list():
        ...
        valid_match = re.search(
            re.compile(pattern.format(thanks), re.IGNORECASE), message
        )
        invalid_quotes = re.search(
            re.compile(quotes_pattern.format(any_char, thanks, any_char)), message
        )
        invalid_greentext = re.search(
            re.compile(
                greentext_pattern.format(any_char, thanks, any_char), flags=re.MULTILINE
            ),
            message,
        )
        if (
            valid_match is not None
            and invalid_quotes is None
            and invalid_greentext is None
        ):
            return True
    return False
