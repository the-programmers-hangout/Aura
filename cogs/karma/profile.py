...
class KarmaProfile(commands.Cog):
        ...
        if len(args) == 0:
            ...
            if karma is None:
                return_msg += "{} has earned a total of {} karma\n".format(
                    ctx.message.author.name + "#" + ctx.message.author.discriminator, 0
                )
            else:
    ...
    @commands.command(
        brief="get karma profile of a user or yourself",
        usage="{}profile\n{}profile <@!member_id>".format(
            config["prefix"], config["prefix"]
        ),
    )
    ...
    async def build_profile_embed(
        self, karma_member: KarmaMember, guild: discord.Guild
    ) -> discord.Embed:
        """
        ...
        if len(channel_list) > 0:
            ...
            embed.set_field_at(
                index=0, name="**total**", value=str(total_karma), inline=False
            )
            return embed
        else:
