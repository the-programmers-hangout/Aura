...
class KarmaProfile(commands.Cog):
    ...
    async def karma(self, ctx):
        guild_id: str = config['guild']
        guild = self._bot.get_guild(int(guild_id))
        roles = ctx.message.author.roles
        if len(ctx.message.mentions) == 1 and (roles()['admin'] in [role.name for role in roles]
                                               or roles()['moderator'] in [role.name for role in roles]):
            message = ctx.message
            ...
            karma_member = KarmaMember(config['guild'], ctx.message.author.id)
            karma = self._karma_service.aggregate_member_karma(karma_member)
            await ctx.channel.send('{} has earned a total of {} karma'
