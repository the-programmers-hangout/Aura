...
class KarmaLeaderboard(commands.Cog):
    ...
    @commands.command(
        brief="get a global karma leaderboard or a channel leaderboard, "
        + "optionally you can provide days as an argument",
        usage=leaderboard_usage.format(
    ...
    async def leaderboard(self, ctx, channel_mention="", time_span: int = 0) -> None:
        ...
        if channel_mention == "" or channel_mention == "global":
            if time_span == 0:
                leaderboard = list(
                    self.karma_service.aggregate_top_karma_members(str(guild.id))
                )
                ...
                if len(leaderboard) > 0:
                    ...
                    for document in leaderboard:
                        ...
                        if member is not None:
                            ...
                            embed.add_field(
                                name=f"{count}) " + bold_field.format("deleted user"),
                                value=f"{karma} karma",
                                inline=False,
                            )
                        count += 1
                ...
                leaderboard = list(
                    self.karma_service.aggregate_top_karma_members(
                        guild_id=str(guild.id), time_span=time_span
                    )
                )
                limit = config["leaderboard"]
                embed.title = (
                    f"Top {limit} most helpful people of the last {time_span} days"
                )
                if len(leaderboard) > 0:
                    count: int = 1
                    for document in leaderboard:
            ...
            input_channel_name = ""
            try:
                input_channel = await TextChannelConverter().convert(
                    ctx=ctx, argument=channel_mention
                )
            ...
            if input_channel is None:
                await ctx.channel.send(
                    "Channel does not exist or lacking permissions to view it."
                )
            else:
                if not ctx.message.author.permissions_in(input_channel).view_channel:
                    ...
                    if time_span == 0:
                        leaderboard = list(
                            self.karma_service.aggregate_top_karma_members(
                                str(guild.id), str(input_channel.id)
                            )
                        )
                        limit = config["leaderboard"]
                        embed.title = (
                            f"Top {limit} most helpful people in {input_channel_name}"
                        )
                        if len(leaderboard) > 0:
                            count: int = 1
                            for document in leaderboard:
                                member = self.bot.get_user(
                                    int(document["_id"]["member_id"])
                                )
                                karma = document["karma"]
                                if member is not None:
                                    embed.add_field(
                                    ...
                                    embed.add_field(
                                        name=f"{count}) "
                                        + bold_field.format("deleted user"),
                                        value=f"{karma} karma",
                                        inline=False,
                                    )
                                count += 1
                            ...
                            await ctx.channel.send(
                                "No leaderboard exists for this channel."
                            )
                    else:
                        leaderboard = list(
                            self.karma_service.aggregate_top_karma_members(
                                guild_id=str(guild.id),
                                channel_id=str(input_channel.id),
                                time_span=time_span,
                            )
                        )
                        limit = config["leaderboard"]
                        ...
                        if len(leaderboard) > 0:
                            ...
                            await ctx.channel.send(
                                "No leaderboard exists for this timeframe"
                            )
