...
class KarmaReducer(commands.Cog):
    ...
    async def reset(self, ctx, *, args: str) -> None:
        ...
        for member in member_set:
            if member_has_role(
                member, roles()["admin"]
            ) and ctx.message.author.id != int(config["owner"]):
                ...
                if member_has_role(
                    ...
                    await ctx.channel.send(
                        "Removed all Karma from {}".format(member.mention)
                    )
...
class KarmaBlocker(commands.Cog):
    ...
    @commands.command(
        ...
        usage="{}showblackist".format(config["prefix"], config["prefix"]),
    )
