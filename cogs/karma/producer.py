...
class KarmaProducer(commands.Cog):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if not message.author.bot:
            ...
            if await validate_message(message):
                ...
                if (
                    self.blocker_service.find_member(
                        Member(str(guild_id), message.author.id)
                    )
                    is None
                ):
                    ...
                    if str(config["blacklist"]["dm"]).lower() == "true":
                        log.info(
                            "Sending Blacklist dm to {} in guild {}".format(
                                message.author.id, guild_id
                            )
                        )
                        await message.author.send(
                            "You have been blacklisted from giving out Karma, "
                            "if you believe this to be an error contact {}.".format(
                                config["blacklist"]["contact"]
                            )
                        )
    ...
    async def on_message_edit(
        ...
        if str(karma()["edit"]).lower() == "true":
            ...
            if before_valid and after_valid:
                ...
                log.info(
                    f"Removing karma because message: {after.id} not valid after edit"
                )
                ...
                log.info(
                    f"Adding karma because message: {after.id} is valid after edit"
                )
        ...
        for reaction in reactions:
            if reaction.emoji == reaction_emoji()["karma_gain"]:
                ...
                if reaction.me:
                    # find message id in db
                    if self.karma_service.find_message(str(message.id)) is not None:
                        await self.remove_karma(
                            message, message.guild, "reaction clear"
                        )
    ...
    async def on_reaction_add(
        ...
        """
        if (
            self.karma_service.find_message(str(reaction.message.id)) is not None
            ...
            if reaction.emoji == reaction_emoji()["karma_delete"]:
                if str(karma()["self_delete"]).lower() == "true":
                    log.info(
                        "Removing karma because the karma_delete emoji was clicked by author"
                    )
                    for other_reaction in reaction.message.reactions:
                        await clear_reaction(other_reaction)
                    await self.remove_karma(
                        reaction.message, reaction.message.guild, "self emoji clear"
                    )
    ...
    async def give_karma(self, message: discord.Message, guild: discord.Guild) -> None:
        ...
        for mention in set(message.mentions):
            ...
            if (
                ...
                if (
                    member.id
                    not in self._members_on_cooldown[guild.id][message.author.id]
                ):
                    karma_member = KarmaMember(
                    ...
                    log.info(
                        "{} gave karma to {} in guild {}".format(
                            message.author.id, member.id, guild.id
                        )
                    )
                else:
                    log.info(
                        "Sending configured cooldown response to {} in guild {}".format(
                    ...
                    if str(config["karma"]["time-message"]).lower() == "true":
                        await self.bot.get_channel(message.channel.id).send(
                            "Sorry {}, your karma for {} needs time to recharge".format(
                                message.author.mention, member.name
                            )
                        )

    async def remove_karma(
        self, message: discord.Message, guild: discord.Guild, reason: str
    ) -> None:
        """
        ...
        for mention in set(message.mentions):
            member = mention
            karma_member = KarmaMember(
                guild.id, member.id, message.channel.id, message.id, 1
            )
            ...
            if deletion_result.deleted_count == 1:
                ...
                if single_action_timer is not None and single_action_timer.is_started:
                    ...
                    self._members_on_cooldown[guild.id][message.author.id].remove(
                        member.id
                    )
            await self.log_karma_removal(message, member, reason)

    async def notify_member_gain(
        ...
        if str(karma()["message"]).lower() == "true":
            if str(karma()["self_delete"]).lower() == "true":
                await self.bot.get_channel(message.channel.id).send(
                    "Congratulations {}, you have earned karma from {}. ".format(
                        member.mention, message.author.mention
                    )
    ...
    async def log_karma_removal(
        self, message: discord.Message, member: discord.Member, event_type: str
    ) -> None:
        """
