...
class SettingsManager(commands.Cog):
    ...
    @commands.command(
        ...
        usage="{}config\n{}config [keys] [new_value]\n{}config help [keys]".format(config["prefix"], config["prefix"], config["prefix"]),
    )
                ...
                if args[0] == "help":
                    ...
                    if len(args) == 3:
                        if args[0] in config.keys() and args[1] in config[args[0]].keys():
                            ...
                            write_config()
                            await ctx.channel.send("Configuration parameter {} {} has been changed to {}".format(args[0], args[1], args[2]))
                        else:
    ...
    def build_config_embed(self) -> Embed:
        ...
        config_embed: Embed = Embed(
            title="Aura Configuration Menu", description="Shows all changeable configuration keys " + "and their current values ", colour=embed_color
        )
