...
class Help(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if self.bot.user.mentioned_in(message) and len(message.content) == len("<@!{}>".format(self.bot.user.id)):
            ...
            version_field = "```fix\nVersion: {}\nDiscord.py: {}\nPython: {}```".format(
                version()["aura_version"], version()["discord_version"], version()["python_version"]
            )
    ...
    @guild_only()
    @commands.command(
        brief="show all commands or show help text of a single command", usage="{}help\n{}help (command)".format(config["prefix"], config["prefix"])
    )
...
class KarmaTutor(commands.Cog):
    ...
    @commands.command(brief="explain the karma system to the caller based on the current configuration", usage="{}explain".format(config["prefix"]))
    async def explain(self, ctx) -> None:
        ...
        embed.add_field(name="**General**", value="Karma is a way for you to show gratitude to helpers with karma points.")
        embed.add_field(
            name="**How do I give karma?**",
            value="You can give karma by including one of the following case insensitive keywords:{}".format(keywords)
    ...
    @commands.command(brief="shows the ways aura reacts based on the current configuration", usage="{}reactions".format(config["prefix"]))
