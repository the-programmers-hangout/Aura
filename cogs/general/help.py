...
class Help(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if self.bot.user.mentioned_in(message) and len(message.content) == len(
            "<@!{}>".format(self.bot.user.id)
        ):
            embed: Embed = Embed(colour=embed_color)
            embed.title = self.bot.user.name + "#" + self.bot.user.discriminator
            embed.description = (
                "A bot for handling karma points of non-bot guild members."
            )
            embed.add_field(
                name=bold_field.format("Prefix"), value=config["prefix"], inline=True
            )
            embed.add_field(
                name=bold_field.format("Authors"), value=author_discord(), inline=True
            )
            version_field = "```fix\nVersion: {}\nDiscord.py: {}\nPython: {}```".format(
            ...
            embed.add_field(
                name=bold_field.format("Build Info"), value=version_field, inline=False
            )
            current_time = time.time()
            difference = int(round(current_time - self.start_time))
            uptime = datetime.timedelta(seconds=difference)
            embed.add_field(
                name=bold_field.format("Uptime"),
                value=strfdelta(
                    uptime,
            ...
            embed.add_field(
                name=bold_field.format("Ping"), value=f"{int(latency)} ms", inline=True
            )
            embed.add_field(
                name=bold_field.format("Source"), value=repository(), inline=False
            )
    ...
    @guild_only()
    @commands.command(
        brief="show all commands or show help text of a single command",
    ...
    async def command_info_embed(self, embed: Embed, ctx, args: List[str]) -> Embed:
        ...
        if command is not None and is_executable:
            ...
            embed.description = (
                "Command does not exist or you do not have the permissions to view it"
            )
        return embed
...
class KarmaTutor(commands.Cog):
    ...
    @commands.command(
        brief="explain the karma system to the caller based on the current configuration",
    ...
    async def explain(self, ctx) -> None:
        ...
        embed.add_field(
            ...
            value="You can give karma by including one of the following case insensitive keywords:{}".format(
                keywords
            )
        ...
        embed.add_field(
            name="**What happens after giving out karma?**",
    ...
    @commands.command(
        brief="shows the ways aura reacts based on the current configuration",
