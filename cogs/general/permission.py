...
class PermissionManager(commands.Cog):
    ...
    @commands.command(
        name="getpermission", brief="get the permission of the provided command", usage="{}getpermission (Command)".format(config["prefix"])
    )
    ...
    @commands.command(
        name="showpermission", brief="shows all commands with their currently set permissions.", usage="{}showpermission".format(config["prefix"])
    )
    async def show_permission(self, ctx):
        embed: discord.Embed = Embed(
            color=embed_color, title="Permission Overview", description="shows all commands with their currently set permissions."
        )
